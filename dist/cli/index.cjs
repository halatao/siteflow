...
function checkDoctor(options = {}) {
  const projectRoot = import_node_path4.default.resolve(options.root ?? findProjectRoot());
  const checks = [];
  ...
  addCheck(
    checks,
    import_node_fs2.default.existsSync(packageJsonPath),
    "package.json",
    "package.json exists.",
  ...
  addCheck(
    checks,
    import_node_fs2.default.existsSync(manifestPath),
    "manifest",
  ...
  addCheck(
    checks,
    import_node_fs2.default.existsSync(contentPagesPath),
    "content-pages",
  ...
  addCheck(
    checks,
    import_node_fs2.default.existsSync(uploadsPath),
    "uploads",
...
function resolveManifestPath(projectRoot, manifest, configuredPath) {
  if (import_node_path4.default.isAbsolute(configuredPath)) {
    return configuredPath;
  }
  ...
  return resolveFromRoot(projectRoot, import_node_path4.default.join(manifest.content.root, configuredPath));
}
...
function createDirectory(projectRoot, relativePath, dryRun, summary) {
  ...
  if (!dryRun) {
    import_node_fs3.default.mkdirSync(absolutePath, { recursive: true });
  }
...
function writeFileIfMissing(projectRoot, file, dryRun, summary) {
  ...
  if (!dryRun) {
    import_node_fs3.default.mkdirSync(import_node_path5.default.dirname(absolutePath), { recursive: true });
    import_node_fs3.default.writeFileSync(absolutePath, file.content, "utf8");
  }
...
function updatePackageScripts(projectRoot, dryRun, summary) {
  ...
  if (!dryRun) {
    import_node_fs3.default.writeFileSync(packageJsonPath, `${JSON.stringify({ ...packageJson.data, scripts }, null, 2)}
`, "utf8");
  }
